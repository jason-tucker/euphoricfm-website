...
| F5 | Medium | High | No CI security gates | `.github/workflows/` | No SAST, secret scan, dependency review, or test gate; `pnpm install` ran lifecycle scripts (Dockerfile used `--ignore-scripts`, CI did not). | Added `security.yml` (CodeQL, gitleaks, dependency-review, `pnpm audit`, server tests, `caddy validate`) + dependabot; added `--ignore-scripts` to the build install. | new workflow runs on PR/push | **fixed** |
...
**CI/CD**
- `.github/workflows/security.yml` — CodeQL, gitleaks, dependency-review, `pnpm audit`, server tests, `caddy validate`.
